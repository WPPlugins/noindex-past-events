<?php
/*
Plugin Name: noindex Past Events
Plugin URI:  http://room34.com
Description: Automatically add a "noindex" meta tag to past events' detail pages to prevent them from appearing in search engines.
Version:     1.2.0
...
function r34npe_search_exclude($query) {
	...
	if (!is_admin() && $query->is_search()) {
		...
		$sql = "SELECT `post_id` FROM `" . $wpdb->postmeta . "` WHERE `meta_key` = '_EventEndDate' AND `meta_value` < '" . date('Y-m-d H:i:s') . "'";
...
add_action('pre_get_posts', 'r34npe_search_exclude', 99);
